...
# ==============================================
function installing() {
    if [ ! -x ${bin}/chatgpt ]; then
	echo -e ${red}"
[${green}*${red}] ${green}Installing chatgpt..."${white}
        yes|pkg update && yes|pkg upgrade
        yes|pkg install curl bat
        git clone https://github.com/Darkmux/chatgpt ${opt}/chatgpt
        cd ${opt}/chatgpt
	chmod 777 *.sh
        cp ${execute}/chatgpt ${bin}
        chmod 777 ${bin}/chatgpt
        echo -e ${red}"
